...
export function optionsValidation() {
  ...
  if (OPTIONS.USE_SMTP) {
    if (!SMTP_SENDER_NAME || !SMTP_SENDER_EMAIL || !SMTP_PASSWORD || !SMTP_USERNAME || !SMTP_HOST || !SMTP_PORT) {
      throw Error(
        'missing env config options: SMTP_SENDER_NAME, SMTP_SENDER_EMAIL, SMTP_PASSWORD, SMTP_USERNAME, SMTP_HOST, SMTP_PORT',
      );
    }
  }
